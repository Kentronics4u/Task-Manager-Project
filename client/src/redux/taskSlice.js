...
export const addTask = (task, id) => async (dispatch) => {
  ...
  const response = await axios.post(
    "https://tiny-puce-nematode-tam.cyclic.app/task/add",
    taskData
  );
  if (response) {
...
export const getAllTasks = (token, id) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      "https://tiny-puce-nematode-tam.cyclic.app/task/tasks",
      config
    );
...
export const arrowClick = (item, string) => async () => {
  ...
  try {
    let response = await axios.put(
      `https://tiny-puce-nematode-tam.cyclic.app/task/${taskData.id}`,
      taskData
    );
...
export const deleteItem = (id) => async (dispatch) => {
  let res = await axios.delete(
    `https://tiny-puce-nematode-tam.cyclic.app/task/${id}`
  );
